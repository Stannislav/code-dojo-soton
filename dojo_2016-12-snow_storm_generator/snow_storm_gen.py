...
class Flake():
    ...
    def display(self):
        pygame.draw.circle(self.screen, self.colour, (int(
            self.x), int(self.y)), self.size / 2, self.thickness)
        for n in range(3):
            angle = n * math.pi / 3 + self.intangle
            pygame.draw.line(self.screen, self.colour,
                             (int(self.x - self.size * math.cos(angle)),
    ...
    def move(self):
        ...
        self.x += math.sin(self.angle) * self.speed + \
            math.sin(self.wobbleangle) * self.wobbleamp
...
def draw_snowman(screen):
    pygame.draw.circle(screen, (255, 255, 255),
                       (width / 4, height - 20), 40, 0)
    pygame.draw.circle(screen, (255, 255, 255),
                       (width / 4, height - 75), 30, 0)
    pygame.draw.circle(screen, (255, 255, 255),
                       (width / 4, height - 115), 20, 0)
    pygame.draw.line(
        screen,
        (100, 50, 0),
        (width / 4 - 20, height - 85),
        (width / 4 - 50, height - 100),
        3)
    pygame.draw.line(
        screen,
        (100, 50, 0),
        (width / 4 + 20, height - 85),
        (width / 4 + 50, height - 100), 3)
...
def draw_main(screen):
    ...
    if random.randint(0, 1) == 0:
        my_flakes += [Flake(
            screen,
            (random.randint(0, 3 * width) - width,
             -10 - random.randint(0, 20)),
            def_size) for i in range(5)]
    # draw all flakes
