...
def get_note(pos,ref=ref_freq):
    return ref*math.pow(2,float(pos)/12)
...
def play_on_key():
    while True:
        ...
        if a in tonedict_user:
            play_tone(tonedict_user[a], ampl, 1/speed/3, fs, stream)
    
        if a == b'\x1b': # the 'Escape' key
            break
    ...
    with open("notes2.txt") as f:
        for line in f:
            ...
            for c in line.lower().split():
                play_tone(tonedict_note[c], ampl, 1/speed, fs, stream)
